...
func (ss *SQLStore) GetOrgUsers(ctx context.Context, query *models.GetOrgUsersQuery) error {
	...
	if ss.Cfg.IsFeatureToggleEnabled(featuremgmt.FlagAccesscontrol) {
		filter, err := accesscontrol.Filter(ctx, "org_user.user_id", "users", "org.users:read", query.User)
		if err != nil {
			return err
		}
		whereConditions = append(whereConditions, filter.Where)
		whereParams = append(whereParams, filter.Args...)
	}
